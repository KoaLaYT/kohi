...
ASSEMBLY := engine
EXTENSION := .dylib
COMPILER_FLAGS := -g -fdeclspec -fPIC -std=gnu2x
INCLUDE_FLAGS := -Iengine/src -I$(VULKAN_SDK)/include
LINKER_FLAGS := -g -shared -lvulkan -L$(VULKAN_SDK)/lib \
				-lobjc -framework AppKit -framework QuartzCore \
				-ObjC -dynamiclib -install_name @rpath/bin/lib$(ASSEMBLY)$(EXTENSION)
DEFINES := -D_DEBUG -DKEXPORT

# Make does not offer a recursive wildcard function, so here's one:
#rwildcard=$(wildcard $1$2) $(foreach d,$(wildcard $1*),$(call rwildcard,$d/,$2))

SRC_FILES := $(shell find $(ASSEMBLY) -name *.c -o -name *.m)		# .c files
