...
/**
 * The names of required extensions for this platform.
 */
                                      ...
                                      struct platform_state* plat_state)
{
    ...
    const char** ext_names = platform_get_required_extension_names();
#if defined(_DEBUG)
    darray_push(ext_names,
                &VK_EXT_DEBUG_UTILS_EXTENSION_NAME);  // debug utilities

    KDEBUG("Required extensions:");
    u32 length = darray_length(ext_names);
    for (u32 i = 0; i < length; ++i) {
        KDEBUG(ext_names[i]);
    }
#endif
    create_info.enabledExtensionCount = darray_length(ext_names);
    ...
    // If validation should be done, get a list of the required validation
    // layert names and make sure they exist. Validation layers should only be
    // enabled on non-release builds.
#if defined(_DEBUG)
