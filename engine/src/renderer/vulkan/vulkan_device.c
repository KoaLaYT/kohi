...
b8 physical_device_meets_requirements(
    ...
    vulkan_physical_device_queue_family_info* out_family_info,
...
b8 vulkan_device_create(vulkan_context* context)
{
    ...
    darray_push(ext_names, &VK_KHR_SWAPCHAIN_EXTENSION_NAME);
#ifdef KPLATFORM_APPLE
    darray_push(ext_names, &"VK_KHR_portability_subset");
#endif
    device_create_info.ppEnabledExtensionNames = ext_names;
    device_create_info.enabledExtensionCount = darray_length(ext_names);
...
b8 select_physical_device(vulkan_context* context)
{
    ...
    for (u32 i = 0; i < physical_device_count; i++) {
        ...
        if (result) {
            ...
            // Keep a copy of properties, feature, memory info for later use.
    ...
    vulkan_swapchain_support_info* out_swapchain_support)
{
    ...
    for (u32 i = 0; i < queue_family_count; i++) {
        ...
        if (queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
            // Take the index if it is the current lowest. This increase the
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        KINFO(
                            "Required extension not found: '%s', skipping "
                            "device.",
                            requirements->device_extension_names[i]);
